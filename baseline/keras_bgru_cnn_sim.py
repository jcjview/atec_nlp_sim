...
def get_model(embedding_matrix, nb_words):
    ...
    x1=words_embedding_layer(input1_tensor)
    x1=seq_embedding_layer(x1)
    x1=cnn1d_layer(x1)

    x2 = words_embedding_layer(input2_tensor)
    x2 = seq_embedding_layer(x2)
    x2=cnn1d_layer(x2)
    # pooled_gru_cnn= lambda tensor: cnn1d_layer(seq_embedding_layer(words_embedding_layer(tensor)))
    avg_pool = keras.layers.GlobalAveragePooling1D()
    max_pool = keras.layers.GlobalMaxPooling1D()
    x1=keras.layers.concatenate([avg_pool(x1),max_pool(x1)])
    x2=keras.layers.concatenate([avg_pool(x2),max_pool(x2)])
    # seq_embedding = lambda tensor: [avg_pool(pooled_gru_cnn(tensor)),max_pool(pooled_gru_cnn(tensor))]
