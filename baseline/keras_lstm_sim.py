...
class F1ScoreCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if (self.validation_data):
            ...
            # print(self.validation_data[3].shape)
            # print(self.validation_data[4].shape)
            y_predict=self.model.predict([self.validation_data[0],self.validation_data[1]], batch_size=self.predict_batch_size)
            # y_predict1 = 1 - y_predict
            # y_predict = np.stack((y_predict, y_predict1), axis=1)
            y_predict =  (y_predict>0.5).astype(int)
            print(y_predict.shape)
            print(np.sum(y_predict))
            # y_predict=np.reshape(y_predict,newshape=(-1,))
            # y_predict[0]=1
            # print(y_predict)
            logs['recall'] = accuracy_score(self.validation_data[2], y_predict)
            logs['f1_score'] = recall_score(self.validation_data[2],y_predict)
            print(logs['recall'],logs['f1_score'])
...
for ind_tr, ind_te in skf.split(X_train_q1, y):
    ...
    print(count,"accuracy ", precision)
